...
async function main() {
  ...
  /* The array literal is wrapped in `as Deal[]` to bypass TS's
     "union type too complex" error that fires once the catalog
     crosses ~2k entries. Without the cast, TypeScript tries to
     infer a literal-union type for every property of every entry
     and runs out of work — see https://github.com/microsoft/TypeScript/issues/15188
     The runtime shape is unchanged; this is purely a type-system
     escape hatch. */
