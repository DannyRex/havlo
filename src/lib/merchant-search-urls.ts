...
const MERCHANTS: Record<string, MerchantHandlers> = {
  ...
  "back-market":       { name: "Back Market",       searchUrl: (_q, c) => `https://www.backmarket.com/${c === "uk" ? "en-gb" : c === "de" ? "de-de" : c === "fr" ? "fr-fr" : "en-us"}`, homepage: "https://www.backmarket.com/en-us" }, // /<locale>/search?q= now 404s (user-verified Jun 2026) and Datadome bot-walls every deep link; real products live at /<locale>/p/<curated-slug> which we can't build from a Google Shopping title, so route to the locale storefront root (always live) rather than ship a guessed 404
