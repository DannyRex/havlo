...
const RULES: Array<{ pattern: RegExp; slug: string; reason: string }> = [
  ...
  // ── Appliances ──
  //    Standalone category again (June 2026 — split back out of the
  //    May 2026 Electronics merge now that appliance inventory is
  //    healthy). These title rules are the SSOT for routing: ingest runs
  //    categoryDisagreesWithTitle, so a fridge / washer / air-fryer /
  //    vacuum / AC / generator title is corrected to "appliances" even
  //    when a provider's own category guess says electronics. Migration
  //    0068 re-tags the existing rows that 0065 had folded into
  //    electronics.
  { pattern: /\b(refrigerator|fridge|freezer|washer|washing\s*machine|tumble\s*dryer|dryer|dishwasher|microwave|oven|range|cooktop|stove)\b/i, slug: "appliances", reason: "major appliance" },
