...
export default function NotFound() {
  ...
  return (
    <section className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 text-center">
      ...
      <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-surface-2 border border-border text-ink-3 text-xs font-semibold uppercase tracking-[0.12em]">
        <span
          aria-hidden
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {popular.map((c) => (
            <Link
              key={c.slug}
              href={`/deals/${c.slug}`}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-surface-2 border border-border text-ink-2 text-sm hover:border-border-strong hover:text-ink transition-colors"
            >
              {c.name}
